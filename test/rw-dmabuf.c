...
{
	size_t f_offs[] = { 0, dev->block_size, 2 * dev->block_size, 7 * dev->block_size};
	size_t buf_offs[] = { 0, dev->block_size, 2 * dev->block_size, 7 * dev->block_size};
	...
	for (int foff_idx = 0; foff_idx < ARRAY_SIZE(f_offs); foff_idx++) {
		for (int buf_idx = 0; buf_idx < ARRAY_SIZE(buf_offs); buf_idx++) {
			for (int size_idx = 0; size_idx < ARRAY_SIZE(io_sizes); size_idx++) {
				size_t f_off = f_offs[foff_idx];
				size_t buf_off = buf_offs[buf_idx];
...
static int test_device(const char *dev_name)
{
	struct io_uring_rsrc_update2 up;
	struct io_uring_regbuf_desc rd;
	struct io_uring ring;
	size_t max_io_size = 128 * 1024;
	struct t_dev dev;
	struct t_buf dmabuf;
	int ret;
