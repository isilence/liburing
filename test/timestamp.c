...
static void validate_key(int tskey, int tstype, struct ctx *ctx)
{
	int stepsize;
...
static int do_test(struct ctx *ctx)
{
	...
	ret = io_uring_queue_init(32, &ring, IORING_SETUP_CQE32);
	if (ret)
	...
	io_uring_for_each_cqe(&ring, head, cqe) {
		...
		ts = (void *)(cqe + 1);
		tstype = cqe->flags >> IORING_TIMESTAMP_TSTYPE_SHIFT;
		tskey = cqe->res;
		hwts = cqe->flags & IORING_CQE_F_TIMESTAMP_HW;
...
static void do_main(int family, int proto)
{
	struct ctx ctx;
	int ret;
...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		fprintf(stderr, "expected hostname argument\n");
		return 1;
	}

	resolve_hostname(argv[1], dest_port);
	do_listen(PF_INET6, SOCK_STREAM, &daddr6, sizeof(daddr6));
	do_main(PF_INET6, SOCK_STREAM);
	return 0;
}
